...
if (isAdmin()) {
    redirectWithMessage('../admin/dashboard.php', 'error', 'Already logged in');
}
if (isStudent()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            redirectWithMessage('../admin/dashboard.php', 'success', 'Login successful');
        } else {
        ...
        <div class="hidden lg:block relative w-0 flex-1 gradient-bg">
            <div class="absolute inset-0 flex items-center justify-center p-12">
                <div class="text-white max-w-md">
                    <h1 class="text-4xl font-bold mb-6 animate__animated animate__fadeIn">Admin Portal</h1>
                    <p class="text-xl opacity-90 animate__animated animate__fadeIn animate__delay-1s">
                        Manage the Student Dormitory System
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
            <div class="mx-auto w-full max-w-md lg:w-96">
                <div class="text-center lg:text-left">
                    <h2 class="text-3xl font-extrabold text-gray-900">Admin Sign In</h2>
                </div>
